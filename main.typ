...
#align(center)[= Transgenerational effects of inherited DNA damage in _C. elegans_]
\
...
These lesions might be bypassed by error-prone polymerases or improperly repaired, leaving behind mutations.
...
Because each strain was derived from a single F1 animal — the offspring of a mutagenized P0 — all mutations should be biallelic. 
The presence of a third allele implies that a DNA lesion was transmitted to the F1 zygote, evaded repair for at least five embryonic cell divisions, and was repeatedly bypassed by error-prone polymerases during gametogenesis. 
// As a result, two distinct alleles were incorporated opposite the same lesion, arose in different gametes and manifested as MAVs in the sequenced F2 progeny.
// Lesions present in a parental gamete are transmitted to an F1 zygote, persist for at least five embryonic cell divisions, and later serve as templates for the incorporation of multiple _de novo_ alleles during gametogenesis.
Gametic mosaicism then manifests as multi-allelism in the sequenced progeny of those F1s. 
Given the developmental lineage of #cel, we suspect that many more lesions are inherited than is suggested by MAV prevalence.
// Unlike prior reports of transgenerational DNA damage, in which inherited lesions trigger epigenetic responses that eventually cause embryonic lethality, 
Our results demonstrate that inherited lesions serve as durable templates for new, heritable variation. 
Inherited DNA lesions may be an underappreciated source of mosaicism and genetic diversity in humans and other metazoans.
// Our results demonstrate that DNA lesions are inherited from parent to offspring in #cel, and become durable, transgenerational templates for _de novo_ genetic variation.
// DNA damage in the early embryo may be a source of multi-allelic mutations in humans, as well.

== Introduction

Myriad sources of damage, from reactive oxygen species to chemotherapeutic agents, leave behind lesions on DNA molecules.
...
These high-fidelity polymerases are often unable to replicate across lesions, triggering the recruitment of specialized translesion synthesis (TLS) polymerases @Sale2013-gj @Minocha2026-bv @Waters2009-wc.
Owing to their unique structural features, including large, accommodating active sites and a lack of exonuclease proofreading abilities, TLS polymerases are capable of synthesizing over DNA adducts, abasic sites, and other lesions that might normally lead to polymerase stalling and replication fork collapse @Gyure2023-ni @Kusumoto2002-re @Haracska2000-ac @Fischhaber2002-rz @Prakash2005-od.
However, the permissive nature of TLS polymerases comes at a cost.
Translesion synthesis occurs with low fidelity, and incorrect nucleotides are frequently incorporated opposite lesions  @Matsuda2001-ud @Vaisman2017-jw @Waters2009-wc.
...
Aitken _et al._ (2020) @Aitken2020-fy previously documented a phenomenon called "lesion segregation," in which DNA lesions persist unrepaired for at least one cell division following a mutagenic insult. 
As long as a lesion evades DNA repair machinery, it can serve as a template for replicative enzymes, including error-prone translesion synthesis polymerases.
...
In 2020, Volkova _et al._ @Volkova2020-gt mutagenized 54 _Caenorhabditis elegans_ strains with 12 DNA damaging agents. 
...
Instead, we believe that MAVs arise when a lesion occurs in a parental (P0) gamete, is transmitted to an F1 zygote, and persists for long enough during embryogenesis to serve as a template for the incorporation of multiple _de novo_ alleles. 
Given the invariant developmental lineage of #cel @Sulston1977-gh @Sulston1983-zz, we estimate that an inherited lesion must persist for at least 5 embryonic F1 cell divisions in order to generate multi-allelism in the gametes of that F1. 
Certain mutagens appear to cause greater levels of multi-allelism than others, though the stochastic nature of post-zygotic lesion segregation and the effects of survivor bias make it difficult to confidently interpret these differences.
Nevertheless, our results demonstrate that DNA damage can persist from generation to generation in #cel. 
...
After genotoxin treatment and sequencing, #vlk performed read alignment and called single-nucleotide, insertion/deletion (indel), and structural variants (SVs).
...
Here, we limit our analyses to the subset of SNVs observed in strains treated with three alkylating agents: dimethyl sulfate (DMS), ethyl methanesulfonate (EMS), and methyl methanesulfonate (MMS).
...
We further filtered the SNVs in DMS, EMS, and MMS-treated strains to remove variants in annotated low-complexity sequences (*Materials and Methods*), producing a final callset of 68,796 alkylation-induced SNVs (@fig-2).
...
We found almost a thousand ostensibly biallelic SNVs with at least one read supporting a "third allele," though the number of candidate MAVs declined substantially if we required additional reads as evidence (@fig-3).
To obtain a high-confidence MAV callset, we required candidate MAVs to be supported by at least 3 sequencing reads, leaving us with 139 MAVs out of 68,796 total SNVs (\~0.2%).

The #vlk strains were sequenced with two different Illumina platforms (HiSeq 2000 and HiSeq X Ten).
Nearly all of the strains processed on the HiSeq X Ten were sequenced to an average of \~30X depth, while those processed on HiSeq 2000 were sequenced to an average of \~80X (#link(<fig-s1>)[Supplementary Figure 1]).
...
#figure(
  image("fig/Figure 3.png", width: 100%), caption: [*Evidence for multi-allelism in mutagenized #cel strains.* *a)* Diagram of aligned Illumina sequencing reads at a single-nucleotide variant observed in `CD0443a`, an _xpf-1_ mutant strain treated with 0.4mM MMS, and an untreated control strain. Sequencing reads are sorted by start position and colored by read orientation (light grey: aligned to the forward strand, dark grey: aligned to the reverse strand). Read bases aligned to the mutated reference nucleotide are colored. For simplicity of visualization, we show a maximum of 50 reads in each subplot. Legend shows the count of each allele aligned to the forward or reverse strand. *b)* Dotted vertical line shows the number of SNVs at which we observed >=3 reads supporting a third allele. In each of 100 trials, we calculated the number of SNVs at which we observed evidence for multi-allelism (>= 3 reads supporting a non-reference allele) using a random strain's sequencing reads. The distribution of "spurious" MAVs identified in each trial is shown as a grey histogram. *c)* We performed the random sampling procedure described in *b)* using varying thresholds on the number of reads required to support a candidate MAV. The blue line shows the number of empirical MAVs identified at each read threshold. The red line shows the enrichment of empirical vs. spurious MAVs identified at each threshold, calculated by dividing the total number of empirical MAVs by the average number of spurious MAVs identified across 100 random sampling trials.]
) <fig-3>

We were initially suspicious that MAVs were simply a result of sequencing errors or alignment artifacts.
In each DMS, EMS, or MMS-treated strain, we calculated the number of SNVs at which 3 or more reads supported a non-reference allele in a random strain (#link(<box-2>, "Box 2"), *Materials and Methods*).
...
To identify "empirical" multi-allelic variants (MAVs), we first searched for multi-allelic evidence at the set of $M$ SNVs (where $|M| = 68,796$) in 691 strains treated with DMS, EMS, or MMS. 
...
For a given strain with $S$ ostensibly biallelic SNVs, we randomly sampled a different strain's read alignments (from the complete collection of $n$ = 2,712 strains, excluding those with the same genotype as the focal strain, as some _de novo_ mutations may have fixed in a strain prior to genotoxin treatment).
...
Since a random strain should be homozygous for the reference allele at every variant observed in the focal strain, we considered any non-reference evidence to be evidence for a "spurious" MAV.
...
We observed an average of 3.3 spurious MAVs per random sampling trial, and never observed more than 18 spurious MAVs in a single trial. 
Even after removing SNVs in annotated low-complexity regions (*Materials and Methods*), we found that spurious MAVs occurred in lower-complexity sequences than "empirical" MAVs (two-sided Kolmogorov-Smirnov $p = 3.0 times 10^(-3)$; #link(<fig-s2>)[Supp. Fig. 2]).
...
We provide a table of all observed MAVs observed in Supplementary Table 1.
...
#figure(
  ...
  [*Lesions must segregate unrepaired into the $Z_2$ or $Z_3$ cell to generate multi-allelism.*
  ...
  *a)* Following genotoxin treatment in a young adult (YA) worm, a lesion (shown as a red triangle) is generated in a haploid gamete.
  ...
  A damage-free oocyte is then fertilized by the sperm cell that harbors a DNA lesion.
  *b)* The sperm-derived lesion could persist during the first few F1 embryonic cell divisions.
  If it is not repaired and undergoes error-free synthesis, the lesion could segregate into either the $A B$ or $P_1$ cell (the two products of the first embryonic cell division).
  If the lesion-containing strand continues to segregate into germline progenitor cells ($P_2$, $P_3$, and $P_4$) without being repaired, it could template the incorporation of multiple unique derived alleles during gametogenesis.
  Here, the lesion templates the incorporation of a #wrong_c allele during the $P_3$ #sym.arrow $P_4$ division, and a #wrong_g allele during the $P_4$ #sym.arrow $Z_2 / Z_3$ division.
  Critically, if the lesion-containing strand segregates into the $A B$ cell (or any of the non $P$-lineage cells), it could generate multi-allelism in the soma, but _not_ in the germline.
  Similarly, if the lesion templates the incorporation of an incorrect base (in this example, a #wrong_c allele) but either a) segregates into the somatic lineage or b) undergoes error-free repair prior to the $P_4$ #sym.arrow $Z_2 / Z_3$ cell division (see inset), the F1's gametes will be mosaic for both the *`T`* and #wrong_c alleles, and the *`T`* #sym.arrow #wrong_c mutation will appear biallelic in future generations.
  In this toy example, we only depict the fate of the chromosome donated by the parental sperm cell.
  Recall that each cell also possesses a copy of the oocyte-derived chromosome — with a T:A base pair at the focal site — that contributes to the eventual population of F1 gametes.
]
)<fig-5>



Let's next imagine that in the mutagenized P0, a mature, haploid sperm cell harbors a DNA lesion on a single strand (@fig-5).
...
At minimum, in order to generate multi-allelism, a lesion-containing strand would need to segregate into the $Z_2$ or $Z_3$ cell without being repaired (@fig-5).
If the lesion segregates into the somatic lineage or undergoes error-free repair prior to the $P_4 #sym.arrow Z_2 / Z_3$ cell division — and never templates the incorporation of an incorrect base — the F1 gametes will only possess a single allele at that site, and any evidence of an inherited lesion will be lost.
But what happens if an incorrect base _is_ incorporated opposite the lesion?

For example, let's consider the simplest possible way for an inherited lesion to generate multi-allelism in the next generation.
First, imagine that the lesion undergoes error-prone translesion synthesis (TLS) and a #wrong_c allele is incorporated opposite the lesion during the $P_3 #sym.arrow P_4$ division.
If the lesion is again bypassed by error-prone TLS during the $P_4 #sym.arrow Z_2 / Z_3$ division, both the first and second incorrect bases ( #wrong_c and #wrong_g ) will be present in mature gametes.
For the sake of example, we assume that error-prone TLS is the mechanism by which mutations are introduced opposite lesions; one could also imagine other ways for _de novo_ mutations to occur, though these alternative processes might result in the lesion being removed. 
For example, during the $P_4$ #sym.arrow $Z_2 / Z_3$ division, nucleotide excision repair machinery might excise the lesion and fill the gap with an incorrect nucleotide.

In this example, if the lesion is repaired prior to the $P_4 #sym.arrow Z_2 / Z_3$ cell division, only a single incorrect base (the #wrong_c allele) will exist in the germline and the offspring of the F1 would appear to be _biallelic_ (see inset in @fig-5).
If it segregates into a somatic precursor cell during any of the first four embryonic cell divisions, the lesion-containing strand could generate multi-allelism, but those multiple alleles would only exist in the soma and would not be present in sequenced F2 progeny.
We believe that this simple model, in which a lesion-containing strand persists for at least five embryonic cell divisions, explains the presence of MAVs in the #vlk dataset.
...
While we've long known that parental DNA damage is repaired by the early embryo @Generoso1979-oa, our work demonstrates that inherited lesions serve as durable, transgenerational templates for _de novo_ mutagenesis, and suggests a number of exciting new directions.

=== Prior evidence for the transgenerational effects of DNA damage in #cel

Wang et al. @Wang2023-lg recently demonstrated that paternal exposure to ionizing radiation leads to transgenerational embryonic lethality in #cel.
...
The authors concluded that DNA damage — primarily in the form of double-stranded breaks (DSBs) — was transmitted from P0 sperm cells to the zygotes of the F1 generation.
...
In the F1 germline, these damaged chromosomes triggered the formation of heterochromatin, preventing error-free mechanisms (like homologous recombination) from repairing DSBs and leading to embryonic lethality in F2s @Wang2023-lg. 
These results demonstrated that radiation-induced DNA damage persists from parental gametes to fertilized zygotes, and that inherited DNA damage can cause epigenetic changes that significantly impact the viability of future generations.
Here, we describe a related — but distinct — phenomenon, in which single-nucleotide DNA lesions are themselves inherited from generation to generation, serving as recurrent templates for _de novo_ mutations and generating heritable genetic diversity.
...
Thus, we can safely assume that a given MAV is caused by the presence of a single lesion on a single DNA strand in a zygote.
...
Thus, the probability that a lesion is inherited by the $P_4$ cell (assuming it is not repaired) is simply $0.5^4 = 0.0625$.
Lesion segregation in the early zygote is therefore stochastic, depending on the vagaries of Mendelian segregation.

// === "Second-hit" germline mutations are unlikely to explain multi-allelism

// We also considered the possibility that MAVs might occur via "second hit" mutations at previously mutated sites.
// For example, a mutagenized P0 might transmit a _de novo_ single-nucleotide mutation to one of its F1 offspring.
// If a second _de novo_ mutation (DNM) occurred at that same site in the F1's germline — a consequence of the baseline germline mutation rate, rather than error-prone bypass over a persistent lesion — both _de novo_ alleles might be observed in the F1's progeny. 
// However, we believe this phenomenon is unlikely to explain the prevalence of MAVs in the #vlk dataset.

// First, let's consider the naive probability of a "second hit" occurring at a previously mutated site.
// The single-nucleotide _de novo_ germline mutation rate in #cel is approximately $mu_g = 1 times 10^(-8)$ per base pair, per generation.
// Assuming a mutagenized P0 transmits $M$ DNMs to an F1, and that F1 produces $N$ offspring, the probability that a second DNM occurs at one of those $M$ sites is simply $P = 1 - (1 - mu_g)^(N times M)$.
// If $M = 500$ (the median number of DNMs per mutagenized strain) and $N = 300$ (the typical fecundity of a wild-type hermaphrodite), $P = 1.5 times 10^(-3)$.
// However, this probability only describes the likelihood that a single F2 possesses the "second hit" mutation.

// Each strain's sequencing data were derived from the pooled progeny of a single F1 animal.
// Thus, in order to observe multiple reads of support for a "third" allele  (#link(<fig-s2>)[Supp. Fig. 2]), that "third" allele would need to be present in many of the F1's offspring.
// As shown in @fig-5, we hypothesize that error-prone bypass of an inherited lesion could generate multi-allelic mosaicism among the mature gametes of an F1.
// For a "second hit" mutation to generate a similar level of mosaicism, the mutation would need to occur during gametogenesis in the F1's germline.



// Thus, a "second hit" must occur early enough during gametogenesis to manifest in multiple mature gametes.
// Moreover, we observed many strains with >1 MAVs, and at least one strain with 5.
// The probability of a "second hit" occurring at more than one site is correspondingly lower.
// Thus, we believe the most parsimonious explanation for detectable MAVs is the persistence of DNA lesions during F1 development, as described in @fig-5.
...
// If $M = 500$ (the median number of DNMs per mutagenized strain) and $N = 300$ (the typical fecundity of a wild-type hermaphrodite), $P = 1.5 times 10^(-3)$.
// However, this probability only describes the likelihood that a single F2 possesses the "second hit" mutation.
// On average, we observed nearly 8 reads of support for the the "third" allele at multi-allelic sites (#link(<fig-s2>)[Supp. Fig. 2]).  
// Because the #vlk sequencing data were generated from the pooled progeny of a single F1, that "third" allele would need to be present in many of those progeny.
// In strains with loss-of-function mutations in DNA repair or replication genes, $mu_g$ might be up to 100 times higher, and $P$ could be closer to $1 - (1 - 100mu_g)^(N times M) = 0.22$.
// While this probability is much greater, it only describes the likelihood that a single F2 will possess the "second hit" mutation.

// In order to detect a "third" allele in the #vlk sequencing data, the "second hit" would either a) need to occur early enough during gametogenesis to manifest in >1 mature gamete or b) that the F2 with both _de novo_ alleles contributes an unusually high proportion of offspring to the next generation.
// Moreover, we observed many strains with >1 MAVs, and at least one strain with 5.
// The probability of a "second hit" occurring at more than one site is correspondingly lower.
// Thus, we believe the most parsimonious explanation for detectable MAVs is the persistence of DNA lesions during F1 development, as described in @fig-5.
...
If it segregates into a $Z_2$ or $Z_3$ cell and continues to evade repair, a lesion could also create gametic mosaicism by templating incorrect nucleotides during the later stages of germ cell development.
...
Moreover, if a lesion is bypassed by error-prone TLS and repaired in a subsequent cell division, it might generate _de novo_ mutations that appear biallelic in subsequent generations. 
...
But if the lesion is repaired or removed during the next cell cycle, the gametes produced by that worm would only contain two possible alleles, and its offspring would appear to be biallelic (@fig-5).
Similarly, if an incorrect base were incorporated opposite the lesion during the $P_1$ #sym.arrow $P_2$ (or $P_2$ #sym.arrow $P_3$) division and the incorrect base continued to segregate into the $P$ lineage, the resulting mutation would be fully-resolved in the $P_4$ cell.
Exactly half of the F1's gametes would possess the incorrect base, and the site would appear to be biallelic.
Thus, we suspect that a substantial fraction of _biallelic_ SNVs observed in the sequenced F2+ offspring in #vlk likely result from inherited lesions.
The early zygote probably inherits much more damage than is suggested by MAV prevalence.
// Because they were either mature gametes or actively undergoing meiosis at the time of genotoxin treatment, many mutagenized sperm and oocytes likely carried unrepaired lesions into the zygote.
// If those lesions served as templates for error-prone TLS polymerases during embryogenesis, but were subsequently repaired, they would generate apparently biallelic mutations. 

=== Missing MAVs due to survivor bias
...
Ultimately, then, our ability to detect MAVs depends on at least four distinct factors: 1) the random segregation of lesion-containing strands into germline progenitors, 2) the efficiency of lesion repair in the early embryo (i.e., the duration of lesion persistence), 3) the fidelity of translesion synthesis, and 4) the reproductive success of the F1 and its progeny following mutagen exposure.


=== What mechanisms likely lead to multi-allelism?

As suggested in previous studies of lesion segregation @Aitken2020-fy @Anderson2024-fl, we hypothesize that multi-allelic variants (MAVs) arise from error-prone replication over persistent lesions.
In the span of about two hours, the newly formed #cel zygote undergoes five asynchronous and asymmetrical cell divisions @Encalada2000-ju.
The early embryo is highly sensitive to the timing of these divisions, and cell cycle delays can lead to embryonic lethality @Encalada2000-ju.
#cel embryos therefore rely on translesion synthesis (TLS) polymerases to bypass DNA damage that might otherwise stall replication forks and activate cell cycle checkpoints @Holway2006-pd @Roerink2012-ow @Kermi2019-bc.
As a result, inherited lesions may be able to persist for multiple embryonic cell divisions, and each division presents an opportunity for a new, incorrect base to be incorporated.
We hypothesize that a reliance on TLS for DNA damage bypass makes the early embryo highly susceptible to lesion segregation and the generation of heritable MAVs.  
In worms that have not been treated with genotoxins, it remains to be seen whether endogenous DNA damage is likely to persist, or if damage that accumulates during zygotic genome activation @Butuci2015-xv could cause MAVs, as well.


=== Do some mutagens create more persistent lesions?

Compared to a null expectation, we found that certain mutagens led to a greater enrichment of MAVs than others.
For example, despite causing the highest number of mutations per unit dose (@fig-2), EMS treatment led to a relatively modest enrichment of MAVs.
...
Mutagens like EMS almost exclusively create C:G #sym.arrow T:A mutations, and often generate premature stop codons, strong loss-of-function variants, and null alleles @Kutscher2014-ei @Flibotte2010-an.
EMS mutagenesis may therefore prove more lethal to F2+ progeny, limiting our ability to sequence the full complement of _de novo_ alleles present in the offspring of an EMS-treated P0.
...
Moreover, our analysis in this manuscript deals only with single-nucleotide mutations; the mutagens we analyzed also create small numbers of insertions, deletions, and structural variants @Volkova2020-gt.
...
In this study, we analyzed alkylating mutagens that were applied to P0 hermaphrodites at the early young adult (YA) stage of development @Volkova2020-gt.
In #cel hermaphrodites, spermatogenesis and oogenesis occur in sequence; spermatogenesis begins and ends during the L4 larval stage, while oogenesis begins afterward and continues through adulthood @Hubbard2005-ky.
In early young adults, all sperm cells are therefore mature haploid gametes, and a large pool of oocytes is held at the pachytene stage of meiosis; the latter are fertilized within approximately 24 to 36 hours.
As a result, most F1 offspring in #vlk were fertilized by sperm and oocytes that were mature gametes or pachytene-stage cells, respectively, at the time of mutagenesis.
Therefore, we suspect that MAVs derive from persistent lesions inherited from both sperm and oocytes, though mature sperm may be more susceptible to genotoxin treatment due to the compacted state of their chromatin. 

The timing of genotoxin exposure likely has a significant impact on the likelihood of lesion persistence.
Wang _et al._ reported embryonic lethality in the offspring of males irradiated at the late L4 stage (when spermatogenesis is complete), but not in the offspring of males irradiated at the late L3 or early L4 stage (when spermatogenesis is ongoing) @Wang2023-lg. 
This result suggests that repair processes in a gamete progenitor can correct DNA damage relatively quickly. 
Future experiments, in which male and "feminized" hermaphrodites are separately mutagenized at different life stages, could provide insight into the relative ability of sperm and oocytes to repair DNA lesions caused by alkylating agents.
...
=== Implications for mammalian _de novo_ mutagenesis
...
This result has been attributed to mutations that occur during early embryogenesis.
Because the early embryo relies on maternally-deposited machinery to replicate and repair DNA, older oocytes may contribute more defective machinery, leading to elevated rates of post-zygotic mutation on both maternal and paternal haplotypes @Braude1988-hc @Dobson2004-ks @Zhang2009-am @Wong2016-un @Gao2019-yg.
// Our results demonstrate that in #cel, DNA damage is passed down from P0 gametes to F1 zygotes, providing a pool of lesions that must be repaired during embryonic development.
Post-zygotic maternal repair of paternal lesions — and its role in generating a wide range of both small and structural _de novo_ mutations — has been studied extensively in mice @Generoso1979-oa @Marchetti2008-xv @Marchetti2015-av, though its contribution to human genetic diversity remains poorly characterized.
We speculate that inherited DNA damage may be an underappreciated source of heritable _de novo_ mutations in humans.
However, it remains to be seen whether endogenous processes (or typical levels of exogenous mutagens) generate sufficient levels of DNA lesions to manifest as multi-allelic variants.
...
We are excited to continue investigating the role that persistent, transgenerational DNA damage plays in generating inherited genetic diversity.

== Acknowledgments

We thank Drs. Bettina Meier and Anton Gartner for assisting with data access.
We also thank Dr. Gartner and Dr. Gillian Stanfield for helpful feedback on our initial findings and early drafts of the manuscript. 
We thank members of the Quinlan Lab for helpful discussions and brainstorming.
Dr. Aaron Quinlan is supported by award R01HG012252 from the National Institutes of Health (NIH).
The computational resources used in this study were partially funded by the NIH Shared Instrumentation Grant 1S10OD021644-01A1.
...
We have distributed a Snakemake @Koster2012-jj pipeline that can be used to reproduce all data processing, statistical analysis, and figure generation, which can be accessed at https://github.com/quinlan-lab/lesion-segregation. 
...
However, during Illumina sequencing runs, sample indices can occasionally "hop" from one sample to another on a flow cell, meaning that reads from one sample are incorrectly assigned to another sample. 
To test for the possibility that index hopping could explain the presence of MAVs in our datasets, we iterated over each of the 139 MAVs identified in strains treated with DMS, EMS, and MMS. 
At each MAV, we searched for evidence of the non-reference alleles in every other strain's alignments (n = 2,713) using `pysam` (https://github.com/pysam-developers/pysam).
...
At every candidate multi-allelic variant, we performed a similar test for strand bias using the number of reads supporting each alternate allele.
...
These SNVs were identified using the original alignments for each strain (which, in some cases, were sequenced to very high depth). 
...
#bibliography("Paperpile - References - Aug 5.bib", style: "plos")
