...
These lesions may be incorrectly bypassed by error-prone translesion  polymerases or improperly repaired by DNA repair machinery, leaving behind mutations.
...
By reanalyzing sequencing data from a large #cel mutagenesis experiment, we observed robust evidence for MAVs at a small fraction (\~0.25%) of mutated sites in genotoxin-treated strains. 
...
Myriad sources of damage, from reactive oxygen species to ultraviolet radiation, leave behind damaging lesions on DNA.
...
TLS polymerases are also known to introduce new mutations downstream from lesions in a process called "collateral mutagenesis" @Poti2022-ms @Aitken2020-fy @Anderson2024-fl.
...
In the F1 zygote, maternally-deposited polymerase theta-mediated end joining (TMEJ) machinery attempts to repair this damage, creating _de novo_ structural variants in the process. 
The authors speculated that mature _C. elegans_ sperm, which have a highly-compacted chromatin structure, are inaccessible to DNA repair machinery.
...
Given the perfectly-resolved developmental lineage of #cel \@sulston, we estimate that a lesion present in a P0 gamete must persist for at least 4 embryonic F1 cell divisions in order to generate multi-allelism in the gametes of that F1. 
...
#quote()[
"_The zygotes which lead to the F1 generation provide a single cell bottleneck where mutations of exposed male and female germ cells are fixed before being clonally amplified during...development and passed on to the next generation in a Mendelian ratio._"]
...
In total, the authors identified 135,348 high-confidence single-nucleotide variants (SNVs) in 54 strains treated with genotoxins or maintained in M.A. experiments (@fig-2).
...
We found almost two thousand SNVs with support for a "third allele," though the number of candidate MAVs declined substantially if we required additional supporting reads as evidence (@fig-3).
After stringent filtering (*Materials and Methods*), we required candidate MAVs to be supported by at least 3 sequencing reads.
...
In each strain, we calculated the number of randomly sampled SNVs at which 3 or more reads supported a non-reference allele (*Materials and Methods*; and see @Aitken2020-fy for a similar approach).
// #show figure.where(
//   kind: table
...
We note that our random sampling strategy is imperfect.
In each trial, we randomly sample $N$ mutations from the complete callset, where $N$ is the number of mutations in a given strain. 
In some strains, we are guaranteed to randomly sample sites multiple times over the course of 100 trials.
Moreover, the majority of mutations in the callset are derived from EMS; thus, the "spurious" MAVs will be biased toward sequence contexts that are preferentially damaged by EMS.
Nevertheless, we are confident that most MAVs are "real."
...
Using our stringently filtered MAV callset, we measured the degree to which different genotoxins induced multi-allelic mutations.
Compared to the null expectation, MMS caused the greatest enrichment of multi-allelic variants in #cel, followed by DMS and EMS.
It is challenging to attribute these differences in MAV enrichment to the characteristics of specific mutagens, however. 
...
To answer this question, we can take advantage of the fact that the #cel developmental lineage is perfectly characterized (Sulston).
All #cel gametes are derived from a single germline progenitor cell called $P_4$.
The $P_4$ cell divides to create $Z_2$ and $Z_3$, which localize to the gonad and give rise to all eventual gametes (cite).
...
Because the descendants of the $P_4$ cell, known as $Z_2$ and $Z_3$, exclusively give rise to germ cells, a lesion could also create gametic mosaicism by serving as a template for incorrectly incorporated nucleotides during the later stages of germ cell development.
...
Thus, we suspect that MAV detection is complicated by survival bias; high mutagen doses create more lesions, and these lesions might potentiate multi-allelic variation, but those multiple alleles may not segregate into living offspring.
...
The same appears to be true in _Xenopus_ and _Drosophila_ @Lo-Furno2022-zq @Kermi2015-nd, suggesting that the rapidly dividing early embryo is unable to perform proper quality control and instead relies on TLS to bypass DNA damage that would otherwise stall the cell cycle @Ohkumo2006-em (https://www.sciencedirect.com/science/article/pii/S1097276508008125?via%3Dihub).
...
For example, mutagens like EMS almost exclusively create C #sym.arrow T mutations, and as a result often create stop codons,  strong loss-of-function, and null alleles @Kutscher2014-ei.
...
Nonetheless, future work may be able to uncover the relative amounts of multi-allelism generated by different mutagens.
...
Wang et al. (2023) @Wang2023-lg hypothesize that the compacted chromatin architecture of #cel sperm render them incapable of repairing damage caused by ionizing radiation. 
In this study, we analyzed mutagens that were applied to P0 hermaphrodites at the young adult (YA) stage of development.
At this time point, #cel spermatogenesis is complete, and mature sperm will not undergo any additional cell divisions before they  fertilize F1 zygotes.
Oocytes, on the other hand, begin oogenesis at the YA stage and continually divide into adulthood. 
...
compacted sperm sensitive to ox stress https://journals.biologists.com/jcs/article/126/6/1488/54355/The-presence-of-a-truncated-base-excision-repair?guestAccessKey= https://www.tandfonline.com/doi/abs/10.1179/135100001101536157
...
We performed additional filtering on the SNV calls as follows.
To mitigate the effects of low-quality alignments on MAV identification, we removed any SNVs that overlapped regions annotated by RepeatMasker ().
After filtering, we retained a total of 83,000 SNVs.
